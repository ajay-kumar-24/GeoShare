...
public class ViewFragment extends Fragment implements View.OnClickListener,OnMapReadyCallback,GoogleMap.OnMarkerClickListener, GoogleMap.OnMapClickListener,GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, LocationListener, GoogleMap.OnMyLocationButtonClickListener {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        ar = (FloatingActionButton) view.findViewById(R.id.ar);
...
// add a marker to the map indicating our current position
    ...
    private void showPopup(final Activity context, Point p, Marker marker) {
        try {
            ...
            Intent i = new Intent(getActivity(), post_view_full_page.class);

            i.putExtra("post_sender",marker.getTitle().toString().split("says")[0]);
            i.putExtra("post",marker.getTitle().toString().split("says")[1]);
            startActivity(i);
...
//            popup.showAtLocation(layout, Gravity.NO_GRAVITY, p.x + OFFSET_X, p.y + OFFSET_Y);
    ...
    public void get_user_id(){
        ...
        if (extras != null) {
            user_id = extras.getInt("user_id");
            //System.out.println("user id in new activity is "+user_id+" "+ulat+" "+ulon);
    ...
    public void onLocationChanged(Location location) {
        ...
        try {
            ...
            for(int i =0; i< res.length();i++)
            {
                ...
                plat = Double.parseDouble(obj.getString("lat"));
                plon = Double.parseDouble(obj.getString("lon"));
                post = obj.getString("post");
                post_sender = obj.getString("user");
                System.out.println("plat is "+plat);
                System.out.println("plon is "+plon);
                System.out.println(post);
                System.out.println(post_sender);
                LatLng loc = new LatLng(Double.parseDouble(obj.getString("lat")),Double.parseDouble(obj.getString("lon")));
                mMap.addMarker(new MarkerOptions()

                        .position(loc)
                        .title(post_sender+" says "+post)
    ...
    public void onClick(View v) {
        Intent i = new Intent(getActivity(), TextReco.class);
        i.putExtra("plat",plat);
        i.putExtra("plon",plon);
        i.putExtra("post",post);
        i.putExtra("user",post_sender);
